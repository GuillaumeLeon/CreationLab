...
<div class="container row post mt-3 mb-3 shadow">
  <div class="corps">
    <div class="info" style="justify-content: space-between">
      <div class="vote">
	<button type="button" class="btn <?= $value['post_id']?> upvote" <?php if($upvoted){ echo "style='color:green;'"; } ?>
	  id="<?= $value['post_id']?>" onclick="upvote(this.id)"><i class="fa fa-arrow-up"></i></button>
	  <div class="numberVote <?= $value['post_id']?>"><?= $upvote_nb[0] - $downvote_nb[0] ?></div>
	  <button type="button" class="btn <?= $value['post_id']?> downvote" <?php if($downvoted){ echo "style='color:red;'"; } ?>
